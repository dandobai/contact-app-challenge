...
async function submit() {
  ...
  try {
    ...
    // Sikeres login után irány a kontaktok
...
<template>
  <div class="max-w-sm mx-auto mt-16 flex flex-col gap-6 text-white">
    <h1 class="text-3xl font-semibold text-center tracking-tight text-white">Login</h1>

    <transition name="fade">
      <p v-if="error" class="text-red-400 text-sm text-center bg-red-900/20 py-2 rounded-md border border-red-900/50">
        {{ error }}
      </p>
    </transition>
    ...
    <p class="text-center text-sm text-gray-400 font-medium">
      No account? 
      <router-link to="/register" class="text-blue-400 hover:text-blue-300 hover:underline ml-1">
        Register here
      </router-link>
    </p>
  </div>
